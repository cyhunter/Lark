declare module lark.player {
    ...
    const enum TextFieldFlags {
         ...
         * 字体信息的字符串形式发生改变。true
         */
        FontStringChanged = 16384,
        /**
         * 文本行列表发生改变
         */
        TextLinesChanged = 32768,
        /**
...
declare module lark.player {
    ...
    class Ticker {
         ...
         */
        static $invalidateRenderFlag: boolean;
        /**
         * 需要立即刷新屏幕的标志
         */
        static $requestRenderingFlag: boolean;
        constructor();
        private playerList;
        /**
         * 注册一个播放器实例并运行
...
declare module lark.player {
     ...
     */
    var sharedRenderContext: player.RenderContext;
    /**
     * surfaceFactory实例
     */
...
declare module lark.web {
     ...
     * Canvas屏幕适配器
     */
    class WebScreen extends LarkObject {
        /**
         * 创建一个WebScreen实例
         ...
         * @param contentHeight 初始化内容高度
         */
        constructor(container: HTMLElement, canvas: HTMLCanvasElement, scaleMode: string, contentWidth: number, contentHeight: number);
        /**
         * 画布实例
         */
        private canvas;
         ...
         * 更新播放器视口尺寸
         */
        updateScreenSize(player: lark.player.Player, webTouch: WebTouchHandler, webText: WebTextAdapter): void;
    }
}
declare module lark {
    /**
...
declare module lark {
    ...
    class EventEmitter extends LarkObject implements IEventEmitter {
         ...
         * @returns 如果成功调度了事件，则值为 true。值 false 表示失败或对事件调用了 preventDefault()。
         */
        emitWith(type: string, bubbles?: boolean, data?: any): boolean;
    }
}
declare module lark.player {
    /**
...
declare module lark.web {
    class WebTouchHandler extends LarkObject {
        constructor(touch: lark.player.TouchHandler, canvas: HTMLCanvasElement);
        private canvas;
        private touch;
        /**
         * 添加事件监听
...
declare module lark.web {
    class CanvasFactory implements player.SurfaceFactory {
        constructor();
        /**
         * 从对象池取出或创建一个新的Surface实例
         * @param useOnce 表示对取出实例的使用是一次性的，用完后立即会释放。
         */
        create(useOnce?: boolean): player.Surface;
        /**
         * 释放一个Surface实例
         * @param surface 要释放的Surface实例
         */
        release(surface: player.Surface): void;
        /**
...
declare module lark.web {
    class WebHttpRequest extends EventEmitter implements HttpRequest {
        constructor();
        private _xhr;
        /**
         * 本次请求返回的数据，数据类型根据responseType设置的值确定。
...
declare module lark.web {
     ...
     * ImageLoader 类可用于加载图像（JPG、PNG 或 GIF）文件。使用 load() 方法来启动加载。被加载的图像对象数据将存储在 ImageLoader.data 属性上 。
     */
    class WebImageLoader extends EventEmitter implements ImageLoader {
        /**
         * 使用 load() 方法加载成功的 BitmapData 图像数据。
         */
        data: BitmapData;
...
declare module lark {
     ...
     */
    class DisplayObject extends EventEmitter implements player.Renderable {
        /**
         * 创建一个显示对象
         */
        constructor();
        ...
        removeListener(type: string, listener: (event: Event) => void, thisObject: any, useCapture?: boolean): void;
        emit(event: Event): boolean;
        private emitPropagationEvent(event, list);
        willTrigger(type: string): boolean;
    }
}
declare module lark.web {
     ...
     * HTML5 环境下的输入文本
     */
    class WebTextAdapter extends LarkObject implements player.ITextAdapter {
        ...
        private pendingToShowHtmlInput;
        private currentTextInput;
        private currentHtmlInput;
        private singleLineTextInput;
        private multiLineTextInput;
        /**
         * 当用户点击TextInput时，将它设置为正在输入的TextInput对象，HTML text input 会显示出来并获得焦点
         * @param currentTextInput 要输入的TextInput对象
         */
         ...
         */
        updateScaleMode(scaleX: number, scaleY: number, offsetX: number, offsetY: number): void;
        private createHtmlInputs();
        private handleContainerClick;
        private handleHtmlKeyEvent;
        private handleHtmlInputInputEvent;
        private handleHtmlInputBlur;
        private resetHtmlElementStyle(element);
        private resetHtmlInputStyle(element);
...
declare module lark {
    ...
    class TouchEvent extends Event {
         ...
         * 从外部移动进入一个新的显示对象
         */
        static TOUCH_ENTER: string;
        /**
         * 从当前显示对象移出到外部
         */
        static TOUCH_LEAVE: string;
        /**
         ...
         */
        constructor(type: string, bubbles?: boolean, cancelable?: boolean, stageX?: number, stageY?: number, touchPointID?: number, touchDown?: boolean);
         ...
         * @param touchPointID 分配给触摸点的唯一标识号
         */
        static emitTouchEvent(target: IEventEmitter, type: string, bubbles?: boolean, cancelable?: boolean, stageX?: number, stageY?: number, touchPointID?: number, touchDown?: boolean): boolean;
    }
}
declare module lark.player {
    /**
    ...
    class TouchHandler extends LarkObject {
        ...
        private getParentList(target, list);
        /**
         * 触摸结束（弹起）
         * @param x 事件发生处相对于舞台的坐标x
         * @param y 事件发生处相对于舞台的坐标y
